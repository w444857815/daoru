    		...
    		<div id="box">
		    	<c:forEach var="typeList" items="${typeList}">
		 			<c:if test="${typeList != null }">
		 				<input type="checkbox" value="${typeList.id }" /><a href="xsbooks/bookslistpage?typeId=${typeList.id }">${typeList.typeName }</a> ❤❤  <c:if test="${typeList.status==1 }">已完成</c:if> <c:if test="${typeList.status==0 }">未完成</c:if> <br>
		 			</c:if>
		 		</c:forEach>
	 		</div>
