    		...
    		<div id="box">
		    	<c:forEach var="typeList" items="${bookList}">
		 			<c:if test="${typeList != null }">
		 				<input type="checkbox" value="${typeList.id }"><a href="xstypecon/bookslistpage">${typeList.title }</a> ❤❤  <c:if test="${typeList.getStatus==1 }">已完成</c:if> <c:if test="${typeList.getStatus==0 }">未完成</c:if> <br>
		 			</c:if>
		 		</c:forEach>
	 		</div>
