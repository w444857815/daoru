...
<script type="text/javascript">
    ...
    function importData(){
    	...
    	$.ajax({
			...
			success : function(data) {
				...
				if(data.code=='0'){
					$('#thisimportnumstwoa').html("导入成功,本次导入"+data.data.nowTableSize+"条数据");
				}
			},
			error : function() {
				alert("网络错误！");
    ...
    //上传并导入
    function upfilefile(){
    	var files = document.getElementById("uploadFiletwoa").files;
		//文件数量
		//alert(files.length);
		...
		//for(let i=0;i<files.length;i++){
			...
			$.ajax({
	            ...
	            xhr:xhrOnProgress(function(e){
	                ...
	                $('#jindutwoa').attr("style","width: "+percent*100+"%");
	                $('#jindutwoa').html(percent*100+"%");
	                console.log( '=========' +percent);
	            }),
	            processData : false,// 不处理数据
	            contentType : false,// 不设置内容类型
	            /*beforeSend: function(request) {
	                request.setRequestHeader("filepathtwoa", file_path);
	            }, */
	            success:function(data){
	              console.log(data);
	              var jkpbdx = $.parseJSON( data );
	              $('#filepathtwoa').val(jkpbdx.data);
	            },
	            error:function(e){
...
<div class="easyui-layout" data-options="fit:true,border:false">
    ...
    <input type="hidden" id="filepathtwoa">
        <form id="searchUserForm">
            <table>
                <tr>
                    <td style="width: 100px;">导入第一张表:</td>
                    <td>
                    	<input type="file" name="file" multiple id="uploadFiletwoa"/>
                    </td>
                    <td>
...
<!--                         <a href="javascript:void(0);" class="easyui-linkbutton" onclick="ceshi();">测试</a> -->
                    ...
                    <td>
                    	<span id="thisimportnumstwoa"></span>
                    </td>
<!--                     <td> -->
<!--                     	从第几行开始算数据 -->
<!--                     	<select id="rowstart"> -->
...
<!--                     </td> -->
                ...
                <tr>
                	...
                	<td>
                		<div class="progress">
							<div class="bar" style="width: 0%;" id="jindutwoa">0%</div>
                        </div>
                	</td>
                </tr>
<!--                 <tr> -->
