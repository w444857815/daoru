...
<script type="text/javascript">
    ...
    function importData(){
    	...
    	$('#importDatabutoneb').linkbutton('disable');
    	$.ajax({
			type : "post",
			url : "${pageContext.request.contextPath}/importtwo/importdata",
			data : {
				filepath:$('#filepathoneb').val(),
				rowstart:2,
				colsNum:11,
				tableEnd:'one_b'
			},
			...
			success : function(data) {
				...
				if(data.code=='0'){
					$('#thisimportnumsoneb').html("导入成功,本次导入"+data.data.nowTableSize+"条数据");
				}
			},
			error : function() {
				alert("网络错误！");
    ...
    //上传并导入
    function upfilefile(){
    	var files = document.getElementById("uploadFileoneb").files;
		//文件数量
		//alert(files.length);
		...
		//for(let i=0;i<files.length;i++){
			...
			$.ajax({
	            ...
	            xhr:xhrOnProgress(function(e){
	                ...
	                $('#jinduoneb').attr("style","width: "+percent*100+"%");
	                $('#jinduoneb').html(percent*100+"%");
	                console.log( '=========' +percent);
	            }),
	            processData : false,// 不处理数据
	            contentType : false,// 不设置内容类型
	            /*beforeSend: function(request) {
	                request.setRequestHeader("filepathoneb", file_path);
	            }, */
	            success:function(data){
	              console.log(data);
	              var jkpbdx = $.parseJSON( data );
	              $('#filepathoneb').val(jkpbdx.data);
	            },
	            error:function(e){
...
<div class="easyui-layout" data-options="fit:true,border:false">
    ...
    <input type="hidden" id="filepathoneb">
        <form id="searchUserForm">
            <table>
                <tr>
                    <td style="width: 100px;">导入第一张表:</td>
                    <td>
                    	<input type="file" name="file" multiple id="uploadFileoneb"/>
                    </td>
                    <td>
...
<!--                         <a href="javascript:void(0);" class="easyui-linkbutton" onclick="ceshi();">测试</a> -->
                    ...
                    <td>
                    	<span id="thisimportnumsoneb"></span>
                    </td>
<!--                     <td> -->
<!--                     	从第几行开始算数据 -->
<!--                     	<select id="rowstart"> -->
...
<!--                     </td> -->
                ...
                <tr>
                	...
                	<td>
                		<div class="progress">
							<div class="bar" style="width: 0%;" id="jinduoneb">0%</div>
                        </div>
                	</td>
                </tr>
<!--                 <tr> -->
